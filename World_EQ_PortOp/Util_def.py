...
def mvo_quarterly_rebalancing_3(data, asset_map, asset_lower, asset_upper, port_type, 
                                start_year=2020, trading_days_per_quarter=63, 
                                min_train_periods=252, risk_free_rate=0.02):
    """
    **ปรับปรุงใหม่ (MIP Version)**
    คำนวณน้ำหนักพอร์ต MVO แบบรายไตรมาสโดยใช้ Mixed-Integer Programming (MIP)
    เพื่อบังคับเงื่อนไขจำนวนสินทรัพย์ในพอร์ต (10-15 ตัว) และเงื่อนไขอื่นๆ
    """
    weights_dict = {}
    asset_symbols = list(data.columns)
    ...
    for i, rebalance_date in enumerate(rebalance_dates):
        print(f"\n--- Rebalancing {i+1}/{len(rebalance_dates)} (MIP Method) ---")
        print(f"Rebalance Date: {rebalance_date.strftime('%Y-%m-%d')}")
        
        # ... (ส่วนของการเตรียมข้อมูล training/lookback period เหมือนเดิม) ...
        try:
            train_end_pos = original_index.get_loc(rebalance_date) - 1
        except KeyError:
            nearest_dates = original_index[original_index >= rebalance_date]
        ...
        lookback_data = df_reset.iloc[train_start_pos:train_end_pos+1].ffill().bfill()
        
        if len(lookback_data) < 50:
            print(f"❌ Insufficient data for {rebalance_date}")
            continue
            
        # 1. คำนวณ Expected Returns และ Covariance Matrix
        try:
            mu = expected_returns.mean_historical_return(lookback_data)
            S = risk_models.CovarianceShrinkage(lookback_data).ledoit_wolf()
        except Exception as e:
            print(f"❌ Error calculating mu/S: {e}")
            continue

        # 2. (Optional) หา Target Return จากพอร์ต Max Sharpe แบบไม่มีเงื่อนไขจำนวนหุ้น
        # เพื่อใช้เป็นเกณฑ์ขั้นต่ำสำหรับ MIQP
        try:
            ef_temp = EfficientFrontier(mu, S)
            ef_temp.add_sector_constraints(asset_map, asset_lower, asset_upper)
            ef_temp.max_sharpe()
            performance = ef_temp.portfolio_performance(risk_free_rate=risk_free_rate)
            target_return = performance[0]
            print(f"Setting target return from Max Sharpe portfolio: {target_return:.4f}")
        except Exception:
            # หากหา Max Sharpe ไม่ได้ ให้ใช้ค่าเฉลี่ยของ mu เป็นเป้าหมายพื้นฐาน
            target_return = mu.mean()
            print(f"Could not solve for Max Sharpe, using mean return as target: {target_return:.4f}")

        # === ส่วนของการสร้างและแก้ปัญหา MIQP ===
        n_assets = len(asset_symbols)
        w = cp.Variable(n_assets, name="weights")         # น้ำหนัก (ต่อเนื่อง)
        b = cp.Variable(n_assets, boolean=True, name="selection") # ตัวแปรเลือกหุ้น (0/1)

        # Objective: ลดความเสี่ยงของพอร์ตให้ต่ำที่สุด
        objective = cp.Minimize(cp.quad_form(w, S))

        # Constraints
        constraints = [
            cp.sum(w) == 1,
            mu.T @ w >= target_return, # ผลตอบแทนต้องไม่ต่ำกว่าเป้า
        ]

        # -- Individual Asset Bounds --
        weight_upper_bounds = {ticker: 0.30 for ticker in asset_symbols}
        if 'SHV' in weight_upper_bounds:
            weight_upper_bounds['SHV'] = 0.40
        
        upper_bounds = np.array([weight_upper_bounds[ticker] for ticker in asset_symbols])
        constraints += [
            w >= port_type[0], # Lower bound (e.g., 0 for long-only)
            w <= upper_bounds,
        ]

        # -- Sector Constraints --
        sectors = sorted(asset_lower.keys())
        A_sector = np.zeros((len(sectors), n_assets))
        for sec_idx, sector in enumerate(sectors):
            for asset_idx, asset in enumerate(asset_symbols):
                if asset_map.get(asset) == sector:
                    A_sector[sec_idx, asset_idx] = 1.0
        
        sector_lower_bounds = np.array([asset_lower[s] for s in sectors])
        sector_upper_bounds = np.array([asset_upper[s] for s in sectors])

        constraints += [
            A_sector @ w >= sector_lower_bounds,
            A_sector @ w <= sector_upper_bounds,
        ]

        # -- Cardinality (Number of Assets) Constraints --
        constraints += [
            cp.sum(b) >= 10,
            cp.sum(b) <= 15,
        ]
        ...
        # -- Big-M: Link w and b --
        # w_i can be > 0 only if b_i is 1
        # M can be the highest possible weight (upper bound)
        M = 1.0 
        constraints += [ w <= M * b ]
        ...
        try:
            # ECOS_BB เป็น solver ที่มากับ cvxpy และรองรับ MIQP
            problem.solve(solver=cp.ECOS_BB, verbose=False)
            
            if w.value is None or problem.status in [cp.INFEASIBLE, cp.UNBOUNDED]:
                 raise ValueError("MIP could not be solved.")
            
            # ทำความสะอาดน้ำหนักและเก็บผลลัพธ์
            final_weights = {ticker: round(weight, 8) for ticker, weight in zip(asset_symbols, w.value)}
            # ตัด weight ที่น้อยมากๆ ทิ้ง
            final_weights = {k: v for k, v in final_weights.items() if v > 1e-5}
            
            weights_dict[rebalance_date] = final_weights
            num_assets_selected = len(final_weights)
            print(f"✅ MIP solved successfully. Selected {num_assets_selected} assets.")

        except Exception as e:
            print(f"❌ MIP optimization failed for {rebalance_date.strftime('%Y-%m-%d')}: {e}")
            print("Skipping this rebalance period.")
            continue
    
    # สร้าง DataFrame จากผลลัพธ์
    optimal_weights_df = pd.DataFrame.from_dict(weights_dict, orient='index')
    optimal_weights_df = optimal_weights_df.reindex(columns=data.columns, fill_value=0)
