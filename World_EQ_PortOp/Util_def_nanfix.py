...
# ========= NaN-safe asset selection helper (added) =========
def valid_asset_columns_no_nan(df, start_pos, end_pos, required_columns=None):
    """
    Return (valid_cols, dropped_cols) for a given row-window [start_pos, end_pos].
    valid_cols have NO NaN in the entire window. Optionally restrict to required_columns.
    """
    window = df.iloc[start_pos:end_pos+1]
    # columns with no NaN across the window
    if len(window) == 0:
        return [], list(df.columns)
    valid_mask = window.notna().all(axis=0)
    valid_cols = list(window.columns[valid_mask])
    if required_columns is not None:
        valid_cols = [c for c in valid_cols if c in required_columns]
...
def mvo_quarterly_no_sec_cons(data, port_type, 
    ...
    for i, rebalance_date in enumerate(rebalance_dates):
        ...
        # Extract training data
        train_data = df_reset.iloc[train_start_pos:train_end_pos+1].copy()
        # === NaN drop per-window (train-only for MVO) ===
        valid_cols, dropped_cols = valid_asset_columns_no_nan(df_reset, train_start_pos, train_end_pos)
        if len(valid_cols) == 0:
            print("No valid assets after dropping NaN columns in training window, skipping this rebalance. ❌")
            continue
        train_data = df_reset.iloc[train_start_pos:train_end_pos+1, :][valid_cols].copy()
        lookback_data = train_data.copy()
        print(f"Dropping {len(dropped_cols)} column(s) with NaN in training window.")
        lookback_data = train_data.copy()
        if len(lookback_data) < 50:  # Need sufficient data
            print(f"❌ Insufficient data for {rebalance_date}")
            continue
